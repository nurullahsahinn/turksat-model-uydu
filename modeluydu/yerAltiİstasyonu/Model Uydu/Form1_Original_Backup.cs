...
namespace ModelUydu // Proje namespace'i
{
    public partial class Form1 : Form // Form1 sınıfı, Form sınıfından türetilmiş
    {
        ...
        {
            // Form ana renk ayarları - varsayılan Windows form renkleri
            ...
            // OpenGL arka plan rengi
            GL.ClearColor(Color.FromArgb(181, 181, 181)); // Gri arka plan rengi
        ...
        private void Zamanlayici_Tick(object sender, EventArgs e)
        {
            try
            {
                // Seri porttan veri okuma ve ayrıştırma
                string sonuc = OkumaNesnesi.ReadLine(); // Seri porttan bir satır oku

                // Telemetri verisini işle
                IsleVeGuncelleTelemetriVerisi(sonuc);
        ...
        {
            // Manuel ayrılma komutunu gönder
            if (baglantiYoneticisi.KomutGonder("!xT!", true))
            {
        ...
        private void button4_Click_1(object sender, EventArgs e)
        {
            ...
            try
            {
                if (baglantiYoneticisi != null)
                {
                    ...
                    {
                        MessageBox.Show("❌ Bağlantı kapalı!\n\nÖnce 'Test Verisi' seçerek bağlantıyı açın.", 
                            "Bağlantı Hatası", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    }
                }
                else
                {
                    MessageBox.Show("⚠️ Bağlantı yöneticisi başlatılmamış!", 
        ...
        {
            // BaglantiYoneticisi üzerinden bağlantıyı aç
            if (baglantiYoneticisi.BaglantiyiAc())
            {
        ...
        private void buttonMultiSpektralGonder_Click(object sender, EventArgs e)
        {
            if (multiSpektralFiltreYoneticisi != null)
            {
                // Komut gönder ve sonucu kontrol et
                bool basarili = multiSpektralFiltreYoneticisi.KomutGonder();
        ...
        {
            // Textbox değişikliği işlemleri artık MultiSpektralFiltreYoneticisi içinde yönetiliyor
            // TextBox'ta büyük harflerle yazılması kontrolü MultiSpektralFiltreYoneticisi'ne taşındı

            // Burada özel bir işlem yapmaya gerek yok, ancak ileride gerekirse 
            // multiSpektralFiltreYoneticisi üzerinden çağrılabilir
        }
